  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (contents) {
        ...
        if (idx > 0) setPrevContent(contents[idx - 1])
        else setPrevContent(null)
        if (idx < contents.length - 1) setNextContent(contents[idx + 1])
        else setNextContent(null)
      }
      ...
      if (allProgress) {
        setCompletedIds(new Set(allProgress.map(p => p.content_id)))
      }
    }
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] lg:h-screen overflow-hidden -m-6 lg:m-0">
          ...
          {content.pdf_url && (
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-[#384a8f] hover:underline font-medium"
              >
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            {prevContent ? (
              <Link
                ...
                className="inline-flex items-center gap-2 text-sm text-[#384a8f] hover:underline max-w-[45%]"
              >
                <ArrowLeft className="w-4 h-4 flex-shrink-0" />
                <span className="truncate">{prevContent.name}</span>
              </Link>
            ) : <div />}
            {nextContent ? (
              <Link
                href={`/courses/${courseId}/contents/${nextContent.id}`}
                className="inline-flex items-center gap-2 text-sm text-[#384a8f] hover:underline max-w-[45%] text-right"
              >
                <span className="truncate">{nextContent.name}</span>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`text-xs font-medium leading-snug line-clamp-2 ${
                      isCurrent ? 'text-[#384a8f] font-bold' : isCompleted ? 'text-gray-400' : 'text-gray-700'
                    }`}>
                      {idx + 1}. {c.name}
                    </p>
      ...
      <div className="lg:hidden fixed bottom-20 right-4 z-50">
        <button
          onClick={() => setSidebarOpen(!sidebarOpen)}
          className="w-12 h-12 bg-[#384a8f] text-white rounded-full shadow-lg flex items-center justify-center"
        >
                  ...
                  >
                    <div className="flex-shrink-0">
                      {isCompleted ? (
                        ...
                        <PlayCircle className="w-5 h-5 text-[#384a8f]" />
                      ) : (
