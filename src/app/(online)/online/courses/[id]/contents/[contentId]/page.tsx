...
import { ArrowLeft, ArrowRight, CheckCircle2, FileText, ExternalLink, AlertCircle } from 'lucide-react'
...
export default function OnlineContentViewPage() {
  ...
  return (
    <div className="space-y-6 pt-12 lg:pt-0 max-w-4xl mx-auto">
      <Link href={`/online/courses/${courseId}`} className="inline-flex items-center gap-1 text-sm text-[#384a8f] hover:underline">
        <ArrowLeft className="w-4 h-4" /> コンテンツ一覧に戻る
      </Link>
      ...
      {content.slide_url && (
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-bold text-gray-800 flex items-center gap-2"><FileText className="w-5 h-5 text-[#384a8f]" /> スライド資料</h2>
      ...
      {content.pdf_url && (
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <a href={content.pdf_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#384a8f] hover:underline">
      ...
      <div className="flex items-center justify-between pt-4">
        {prevContent ? (
          <Link href={`/online/courses/${courseId}/contents/${prevContent.id}`} className="inline-flex items-center gap-1 text-sm text-[#384a8f] hover:underline">
            <ArrowLeft className="w-4 h-4" /> {prevContent.name}
          </Link>
        ) : <div />}
        {nextContent ? (
          <Link href={`/online/courses/${courseId}/contents/${nextContent.id}`} className="inline-flex items-center gap-1 text-sm text-[#384a8f] hover:underline">
