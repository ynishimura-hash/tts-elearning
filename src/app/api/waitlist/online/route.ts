...
このたびはTTSオンライン有料会員へのお申し込みをご検討いただきありがとうございます。
...
const SUBJECT = '【空き待ち受付】TTSオンライン有料会員 お申し込みについて'
...
async function sendConfirmation(b: WaitlistBody): Promise<boolean> {
  ...
  try {
    ...
    console.error('waitlist-online confirmation failed:', err)
    return false
  }
}
...
export async function POST(request: NextRequest) {
  ...
  // 管理者LINEグループに通知（既存と同じ is_peak_bottom_target = true のグループ）
